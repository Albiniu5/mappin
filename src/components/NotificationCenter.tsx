...
export default function NotificationCenter({ notifications, onLocate, onDismiss, onClearAll }: NotificationCenterProps) {
    ...
    // Trigger visual "ping" when notifications array grows
    useEffect(() => {
    ...
    return (
        <div ref={containerRef} className="fixed top-6 left-1/2 -translate-x-1/2 z-[3000] flex flex-col items-center">
            ...
            <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                className={`relative flex items-center justify-center h-11 px-5 rounded-full border shadow-[0_0_25px_rgba(0,0,0,0.6)] backdrop-blur-xl transition-all group ${isOpen
                    ? 'bg-slate-900 border-slate-600 text-slate-100'
                    : 'bg-slate-900/90 border-slate-700/60 text-slate-300 hover:bg-slate-800 hover:border-slate-500'
                    }`}
            >
