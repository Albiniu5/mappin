...
export async function GET() {
    try {
        ...
        for (let i = 0; i < MAX_LOOPS; i++) {
            ...
            // List of appnames to try (User provided -> Defaults -> Common bypasses)
            const APPNAMES = [
            ...
            // Helper to fetch with rotation
            const fetchWithRotation = async (baseParams: URLSearchParams) => {
                let lastError;
                for (const appname of APPNAMES) {
                    const p = new URLSearchParams(baseParams);
                    p.set('appname', appname);

                    const url = `${BASE_URL}?${p.toString().replace(/%5B%5D=/g, '[]=')}`;
                    try {
                        ...
                        console.log(`Appname '${appname}' blocked: ${lastError}`);
                    } catch (e) {
                        console.error(`Appname '${appname}' failed network`, e);
                    }
                }
                // If all failed
                throw new Error(`All appnames blocked. Last error: ${lastError}`);
            };
            ...
            for (let i = 0; i < MAX_LOOPS; i++) {
                ...
                const params = new URLSearchParams([
                    // Appname set by rotation helper
                    ['profile', 'list'],
                    ['preset', 'latest'],
                ...
                let richData;
                try {
                    richData = await fetchWithRotation(params);
                } catch (e: any) {
                    console.error("Batch fetch failed:", e);
                    // Return 500 so UI shows alert
                    throw new Error("ReliefWeb Blocked: " + e.message);
                }
