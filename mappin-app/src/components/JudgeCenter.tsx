        ...
        <div ref={containerRef} className="relative">
            ...
            <motion.button
                ...
                className={`relative flex items-center justify-center h-12 px-4 rounded-full border transition-all duration-300 gap-2 ${isOpen
                    ? 'bg-indigo-900 border-indigo-400 text-indigo-100 shadow-[0_0_20px_rgba(99,102,241,0.5)]'
                    : judgedConflicts.length > 0
                        ? 'bg-slate-900/90 border-indigo-500/50 text-indigo-300 hover:bg-indigo-900/50 hover:border-indigo-400 shadow-lg'
                        : 'bg-slate-900/80 border-slate-700 text-slate-500 hover:bg-slate-800 hover:border-slate-600'
                    }`}
            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        className="absolute top-full right-0 mt-2 w-[420px] bg-slate-900/95 backdrop-blur-2xl border border-indigo-500/30 rounded-2xl shadow-2xl overflow-hidden"
                    >
                        {/* Header */}
                        <div className="p-4 border-b border-indigo-500/20 bg-gradient-to-r from-indigo-900/30 to-purple-900/30">
                            ...
                            {judgedConflicts.length === 0 ? (
                                ...
                                judgedConflicts.slice(0, 20).map(conflict => (
                                    <motion.div
                                        ...
                                        className="p-3 rounded-xl bg-slate-800/50 hover:bg-slate-800 border border-slate-700/50 hover:border-indigo-500/50 transition-all cursor-pointer group"
                                        onClick={() => {
                                            onLocate(conflict)
                                            setIsOpen(false)
                                        }}
                                    >
                                        ...
                                        {/* Title */}
                                        <h4 className="text-sm font-semibold text-slate-200 group-hover:text-indigo-300 transition-colors line-clamp-2 mb-2">
                                            {conflict.title}
                                        </h4>

                                        {/* Verdict Preview */}
                                        <div className="relative pl-3 border-l-2 border-indigo-500/50">
                                            <span className="text-[10px] font-bold text-indigo-400 uppercase tracking-wider">Verdict:</span>
                                            <p className="text-xs text-slate-400 line-clamp-2 mt-0.5 italic">
